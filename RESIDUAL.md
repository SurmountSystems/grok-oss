...
- **L0 is Surmount GPUI (library shipped 2026-08-31; window not in this tree).** Crate `xai-grok-l0-roster` parses `/running`-shaped JSON, drops prompt text, tool arguments, tokens, and JWTs, tags local or remote host, and names a per-session enqueue drop file. This grok-oss tree does not ship the GPUI window. Do not add an L0 dashboard to the grok-oss TUI. `/dashboard` stays this pager. `/running` stays this machine's grok-oss windows. They must not merge. Session todos stay in this TUI. Leftover that is not this crate: the GPUI window itself lives in the Surmount superproject, operator submodule commits, and `nixos-rebuild` on the operator host. Highest-value next work is that GPUI window in the Surmount superproject. Report: `/home/hunter/.agents/reports/impl-l0-roster.md`.
