...
echo
echo "NOTE: Product seams inside xai-grok-* are not path-restored. Walk the"
echo "seven product land classes in FORK.md Land checklist and"
echo "doc/dev/upstream-regression-filters.md Required land inventory:"
echo "  1. CLI identity (first token grok-oss)"
echo "  2. Config is a surface (/settings rows plus runtime readers)"
echo "  3. grok-oss ledger /spend ingest of usage.jsonl"
echo "  4. DOGE / chrome paint (theme file is not paint)"
echo "  5. Dual-auth hop after included SuperGrok period limits are full"
echo "  6. Last-session on start"
echo "  7. Product skills are not a Python runtime"
echo "Chrome-only, paint-only bubble copy, or skills Python reintroduced is a failed land."
echo "OpenRouter and grok-rate-limit still need a crate-side reconcile:"
...
echo "2. Walk the seven product land classes (catalog + FORK). Assert is files only."
echo "3. Process pins: ./scripts/assert-process-pins.sh  (AGENTS, FORK, RESIDUAL, join script, …)"
echo "4. just ci  (quality only; cannot fail a deleted catalog test)"
