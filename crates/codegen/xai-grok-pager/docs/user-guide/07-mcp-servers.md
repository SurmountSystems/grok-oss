...
grok-oss mcp list
grok-oss mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by grok-oss.
grok-oss mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
grok-oss mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
grok-oss mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
grok-oss mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
grok-oss mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
grok-oss mcp remove github
...
grok-oss mcp doctor github        # Check one server
grok-oss mcp doctor --json        # Machine-readable output
```
...
`grok-oss mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope. Pass `--scope` to say which one to remove.
...
1. The parent session actually connected the server (see Extensions / `grok-oss inspect`)
...
Use `grok-oss inspect` to see all loaded MCP servers and their sources:

```bash
grok-oss inspect          # Human-readable
grok-oss inspect --json   # Machine-readable
```
