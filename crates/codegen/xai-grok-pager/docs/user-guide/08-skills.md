...
Grok OSS also ships **default product skills** in the grok-oss tree at `crates/codegen/xai-grok-bundle/skills/`. On startup (and again after network bundle extract), Grok installs those files into `~/.grok/bundled/skills/`. That live cache is not the source: a later bundle sync can overwrite network-managed files, and default product skills are not listed in the network `manifest.json`. `/polish`, `/subagent`, `/what`, and `/pull-remote-tree` are default Grok OSS skills this way. They are not project packs at `.agents/skills/polish/`, `.agents/skills/subagent/`, `.agents/skills/what/`, or `.agents/skills/pull-remote-tree/`. When you ask to revise a skill in grok-oss, edit `crates/codegen/xai-grok-bundle/skills/`. `/finish` and `/reports` stay host overlay plus pager slash.
...
5. **Version-control project skills.** Commit `.agents/skills/` or `.grok/skills/` to your repository so the whole team benefits. Do not commit `.py` helpers there. User skills in `~/.agents/skills/` or `~/.grok/skills/` stay personal and unshared. `/polish`, `/subagent`, `/what`, and `/pull-remote-tree` are default Grok OSS skills (product tree `crates/codegen/xai-grok-bundle/skills/`, installed into `~/.grok/bundled/skills/`). Do not ship them as project `.agents/skills/polish`, `.agents/skills/subagent`, `.agents/skills/what`, or `.agents/skills/pull-remote-tree` unless you want a project override. When you ask to revise a skill in grok-oss, edit that product tree. The live cache is not the source.
