...
/// Inputs for MCP discovery that ignores personal disable lists (product re-enable UI).
#[derive(Debug, Clone, Copy)]
pub struct McpDiscoveryInputs<'a> {
    pub cwd: &'a std::path::Path,
    pub plugin_registry: Option<&'a ()>,
    pub compat: &'a xai_grok_tools::types::compat::CompatConfig,
}

/// Discover MCP server definitions ignoring personal disable.
pub fn discover_mcp_definitions_ignoring_disable(
    _inputs: &McpDiscoveryInputs<'_>,
) -> std::collections::HashMap<String, agent_client_protocol::McpServer> {
