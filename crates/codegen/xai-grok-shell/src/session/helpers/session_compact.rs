...
/// input path). Tool *use* is forbidden via `tool_choice` (config-driven;
/// typically `none`). Hosted/server-side tools may still be stripped by the
/// caller when the backend rejects `tool_choice: none` with server tools.
///
