...
Before calling `{GOAL_TOOL}(completed: true)`, L1 reads the short on-disk report
that L3 ran the test suite relevant to what changed (the touched packages/modules,
the whole repo suite only when the change is repo-wide). Do not run that suite
on this parent.
